...
from tracking.trackableperson import TrackableObject
...
if conf["use_dropbox"]:
	...
	client = dropbox.Dropbox(conf["dropbox_access_token1"])
...
while True:
	...
	if logFile is None:
		...
		logPath = os.path.join(conf["output_path"], conf["csv_name1"])
		...
		if conf["use_dropbox"] and pos == 0:
			logFile.write("Year,Month,Day,Time,Up,Down\n")
	...
	for (objectID, centroid) in objects.items():
		...
		# to determine direction
			...
			if not to.counted:
				...
				user = "ID {}".format(objectID)
				nan = " "

				if direction < 0 and centroid[1] < H // 2:
					totalUp += 1
					info = "{},{},{},{},{},{}\n".format(year, month, day, time, user, nan)
					logFile.write(info)
					to.counted = True
				...
				# center line, count the object
					...
					totalDown += 1
					info = "{},{},{},{},{},{}\n".format(year, month, day, time, nan, user)
					logFile.write(info)
					to.counted = True
...
if conf["use_dropbox"]:
	client.files_upload(open("output/personlog.csv", "rb").read(), "/pedestrianlog.csv", mode=dropbox.files.WriteMode.overwrite)
