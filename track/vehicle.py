...
from tracking.trackablevehicle import TrackableObject
from tracking.utils import Conf
...
if conf["use_dropbox"]:
	...
	client = dropbox.Dropbox(conf["dropbox_access_token2"])
...
while True:
	...
	if logFile is None:
		...
		logPath = os.path.join(conf["output_path"], conf["csv_name2"])
		...
		if conf["use_dropbox"] and pos == 0:
			logFile.write("Year,Month,Day,Time,Speed (KMPH),ImageID\n")

		# otherwise, we are not using dropbox and this is a empty log
		# file then write the column headings
		elif pos == 0:
			logFile.write("Year,Month,Day,Time,Speed (KMPH)\n")
	...
	for (objectID, centroid) in objects.items():
		...
		# not yet been estimated then estimate it
			...
			if to.lastPoint and not to.estimated:
				...
				to.estimated = True
				passedT += 1
				speedL = float("{:.2f}".format(to.speedKMPH))
	...
	if conf["display"]:
		...
		info = [
			("Total Vehicle Passed", passedT),
			("Last Speed (KMPH)", speedL),
		]

		for (i, (k, v)) in enumerate(info):
			text1 = "{}: {}".format(k, v)
			cv2.putText(frame, text1, (10, H - ((i * 20) + 20)),
...
if conf["use_dropbox"]:
	client.files_upload(open("output/vehiclelog.csv", "rb").read(), "/vehiclelog.csv", mode=dropbox.files.WriteMode.overwrite)
